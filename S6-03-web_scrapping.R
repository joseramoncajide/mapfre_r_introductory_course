...
# Obtención de las opiones (lo vemos por partes)
...
for (j in 1: num_paginas){
  ...
  opiniones_amazon <- rbind(opiniones_amazon, tibble('pagina'=j, 'opinion'=opinion, 'estrellas' = estrellas))
}

opiniones_amazon %>% View('opiniones')

opiniones_amazon <- read_csv('data/opiniones_amazon.csv')
...
sentimiento[[1]]$documentSentiment$score
...
# Con dplyr
opiniones_amazon %>% 
  mutate(puntuacion = puntuaciones_producto,
         sentimiento = case_when(puntuacion > .2 ~  "Positivo",
                                 puntuacion < .2 ~  "Negativo",
                                 TRUE ~  "Neutro")) 
...
article_url <- "https://itb.dk/maerkesager/kapital-til-vaekst/it-branchen-har-faaet-en-flyvende-start-paa-aaret/"
...
# Most of the dictionaries are in English
# get_sentiments("nrc")

# A solution: transalate into Spanish
