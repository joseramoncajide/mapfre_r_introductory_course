...
flights <- read_csv(file = "data/669307277_T_ONTIME.csv.zip")
...
# dplyr 
# nota: , = AND
vuelos_dic_31 <- filter(flights, MONTH==12, DAY_OF_MONTH==31)
...
flights %>%
  ...
  summarise_each(list(media = mean, total = sum), CANCELLED, DIVERTED)
...
flights %>%
  ...
  mutate(across(c(flight_count), pct))
