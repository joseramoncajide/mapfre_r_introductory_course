...
foo <- mutate(flights, AverageSpeed = Distance / AirTime * 60)
select(foo, Distance, AirTime, AverageSpeed)
...
hist(exp(log(na_array_delay$ArrDelay)),breaks = 20)
...
summarise(aa, n_canc = sum(CancellationCode == "D", na.rm = T))
...
flights %>% 
  ...
  group_by(UniqueCarrier) %>% 
  summarise(p_delay_0 = sum(ArrDelay >0)/n(),
            p_delay_10 = sum(ArrDelay >10)/n()) %>% 
  mutate(rank = rank(p_delay_10)) %>% 
  arrange(rank) 
...
flights2 %>% 
  left_join(airports %>% rename("Dest" = "iata"))
...
flights_dt <- flights_dt %>% 
  mutate(
    ...
    arr_time_ok = arr_time + days(overnight),
    sched_arr_time_ok = sched_arr_time + days(overnight)
  )
